...
export default class InnerBlockNode extends Node {
  ...
  getActionBoundary(action: string) {
    ...
    const lastLineNumber = getLineNumberAtOffset(
      this.boundary.end,
      this.editor.document
    );
    const firstNonWhitespaceCharOnLastLine = getFirstNonWhitespaceCharOnLine(
      lastLineNumber,
      this.editor.document
    );
    if (firstNonWhitespaceCharOnLastLine === '}') {
      const previousLineNumber = lastLineNumber - 1;
      const endOfPreviousLine = getLengthOfLine(
        previousLineNumber,
        this.editor.document
      );
      boundaryEnd = getOffsetOfLineAndChar(
        previousLineNumber,
        endOfPreviousLine,
        this.editor.document
      );
